 ...
 */
async function build(payload, options = { path: null, cipherTextOnly: false, typescript:false }) {
...
function decryptFromFile(filePath) {
  ...
  try {
    // eslint-disable-next-line security/detect-non-literal-fs-filename
    const payload = secrets.decrypt(readFileSync(filePath, { encoding: "utf-8" }));
...
function writeFile(filePath, fileContents) {
  try {
    // eslint-disable-next-line security/detect-non-literal-fs-filename
    writeFileSync(filePath, fileContents, { encoding: "utf-8" });
  } catch (error) {
