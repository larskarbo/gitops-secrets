...
if (!(0, fs_1.existsSync)(SECRETS_FOLDER)) {
    // eslint-disable-next-line security/detect-non-literal-fs-filename
    (0, fs_1.mkdirSync)(SECRETS_FOLDER);
}
/**
 ...
 */
function build(payload, options = { path: null, cipherTextOnly: false, typescript: false }) {
    return __awaiter(this, void 0, void 0, function* () {
        const cipherText = secrets_1.default.encrypt(payload);
        ...
        if (options.typescript) {
            lines = [
                ...lines,
                `export const CIPHER_TEXT = "${cipherText}";`,
            ];
        }
        else if (format === "esm") {
            ...
            if (!options.cipherTextOnly) {
                lines = [
                    ...
                    `const CIPHER_TEXT = "${cipherText}";`,
                    "const loadSecrets = () => secrets.loadSecretsFromCipher(CIPHER_TEXT);",
                    "module.Object = { CIPHER_TEXT, loadSecrets };",
                ];
            }
            else {
                lines = [...lines, `const CIPHER_TEXT = "${cipherText}";`, "module.Object = { CIPHER_TEXT };"];
            }
        }
...
function decryptFromFile(filePath) {
    ...
    try {
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        const payload = secrets_1.default.decrypt((0, fs_1.readFileSync)(filePath, { encoding: "utf-8" }));
        return Object.assign(Object.assign({}, payload), { populateEnv: () => secrets_1.default.populateEnv(payload) });
    }
    catch (error) {
...
function writeFile(filePath, fileContents) {
    try {
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        (0, fs_1.writeFileSync)(filePath, fileContents, { encoding: "utf-8" });
    }
    catch (error) {
